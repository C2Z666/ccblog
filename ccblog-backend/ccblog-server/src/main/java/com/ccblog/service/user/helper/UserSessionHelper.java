...
import com.ccblog.utils.MapUtils;
import lombok.Data;
...
public class UserSessionHelper {
    @Component
    @Data
    @ConfigurationProperties("ccblog.jwt") // 把配置文件里面的jwt相关属性全部绑到上面, .的意思是ccblog的子级
    ...
    public String genToken(Long userId) {
        ...
        // 简单说存redis是因为只要jwt签发就一直有效,加入在redis那么管理员可以删除redis，那么哪怕jwt没有失效,也会因为查不到而无法登录
        RedisUtil.set(String.format(RedisPrefixConstant.USER_SESSION,token), String.valueOf(userId), jwtProperties.getExpire() / 1000);
        return token;
    }

    public void removeSession(String session) {
        RedisUtil.remove(session);
    }
    ...
    public Long getUserIdBySession(String session) {
        ...
        try {
            ...
            String user = RedisUtil.get(String.format(RedisPrefixConstant.USER_SESSION,session));
