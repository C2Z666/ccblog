...
public class AliOssUtil {
    ...
    // 注意：新版的AccessKeyId和AccessKeySecret是定义在你的系统环境变量当中的
    ...
    public URL upload(MultipartFile file) throws IOException {
        if(endpoint==null||endpoint.isBlank()){
            throw new IllegalArgumentException(String.valueOf(StatusEnum.OSS_ARGUMENT_FAILED)); // 抛出异常
        }
        ...
        URL signedUrl = null;
        try {
            // 指定生成的签名URL过期时间，单位为毫秒。本示例以设置过期时间为9小时为例。(因为在阿里云里面似乎最多就32400s?)
            Date expiration = new Date(new Date().getTime() + 3600*9 * 1000L);
            // 生成签名URL。
